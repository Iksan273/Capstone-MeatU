...
def predict_freshness():
    try:
        ...
        freshness_model = load_model('path/to/freshness/model.h5')
        freshness_prediction = freshness_model.predict(image_array)[0][0]

        response = {
            'message': 'Freshness prediction successful',
            'freshness_score': float(freshness_prediction),
        }
